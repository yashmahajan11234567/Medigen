...
def test_generic_finder_to_inventory_is_visible_in_dashboard_and_prevents_duplicates(db_session):
    ...
    # Duplicate additions merge quantities instead of raising an error
    added2 = generic_finder.add_to_inventory(user_id=user.id, medicine_id=generic_medicine.id, quantity=3, quantity_unit='tablets')
    assert added2.medicine_id == generic_medicine.id

    # Verify only one inventory row exists and total quantity is updated correctly
    inventory_list = InventoryService(db_session).list_inventory(user_id=user.id)
    assert len(inventory_list.items) == 1
    assert inventory_list.items[0].quantity == 6

    # Dashboard still shows one medicine (same row, merged quantity)
...
def test_dashboard_contract_includes_all_expected_fields_after_medical_record_creation(db_session):
