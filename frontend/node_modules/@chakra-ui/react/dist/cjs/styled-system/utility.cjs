...
var colorMix = require('./color-mix.cjs');
...
function normalizeConfig(config) {
  return entries.mapEntries(config, (property, propertyConfig) => [
    property,
...
function createUtility(options) {
  ...
  const assignColorPaletteProperty = () => {
    const values = mapToJson.mapToJson(tokens.colorPaletteMap);
    register("colorPalette", {
      values: Object.keys(values),
      transform: memo.memo((value) => values[value])
    });
  };
  ...
  const getPropertyValues = (config, resolveFn) => {
    ...
    if (is.isString(values)) {
      return fn?.(values) ?? tokens.getCategoryValues(values) ?? singleton.EMPTY_OBJECT;
    }
    ...
    if (is.isFunction(values)) {
      return values(resolveFn ? fn : tokens.getCategoryValues);
    }
    return values;
  };
  const defaultTransform = memo.memo((prop, value) => {
    return {
  ...
  const transform = memo.memo((prop, raw) => {
    const key = resolveShorthand(prop);
    if (is.isString(raw) && !raw.includes("_EMO_")) {
    ...
    const _colorMix = (value2) => colorMix.colorMix(value2, tokenFn);
  ...
  const resolveShorthand = memo.memo((prop) => {
