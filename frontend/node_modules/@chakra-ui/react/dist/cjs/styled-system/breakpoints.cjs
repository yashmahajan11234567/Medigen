...
function createBreakpoints(breakpoints) {
  ...
  function keys() {
    return uniq.uniq(["base", ...Object.keys(values)]);
  }
  function up(name) {
...
function adjust(value) {
  const computedMax = parseFloat(unitConversion.toPx(value) ?? "") - 0.04;
  return unitConversion.toRem(`${computedMax}px`);
}
function sort(breakpoints) {
  ...
  return entries.map(([name, min], index, entries2) => {
    ...
    return [name, { name, min: unitConversion.toRem(min), max }];
  });
}
