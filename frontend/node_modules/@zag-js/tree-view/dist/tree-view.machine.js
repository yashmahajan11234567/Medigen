...
var tree_view_machine_exports = {};
__export(tree_view_machine_exports, {
  machine: () => machine
});
module.exports = __toCommonJS(tree_view_machine_exports);
var import_core = require("@zag-js/core");
var import_dom_query = require("@zag-js/dom-query");
var import_utils = require("@zag-js/utils");
var import_tree_view = require("./tree-view.collection.js");
var dom = __toESM(require("./tree-view.dom.js"));
var import_checked_state = require("./utils/checked-state.js");
var import_expand_branch = require("./utils/expand-branch.js");
var import_visit_skip = require("./utils/visit-skip.js");
var { and } = (0, import_core.createGuards)();
var machine = (0, import_core.createMachine)({
  props({ props }) {
    return {
      selectionMode: "single",
      collection: import_tree_view.collection.empty(),
      typeahead: true,
      expandOnClick: true,
  ...
  computed: {
    ...
    visibleNodes: ({ prop, context }) => {
      ...
      prop("collection").visit({
        skip: (0, import_visit_skip.skipFn)({ prop, context }),
    ...
    actions: {
      selectNode({ context, event }) {
        ...
        context.set("selectedValue", (prev) => {
          ...
          if (!event.isTrusted && (0, import_utils.isArray)(value)) return prev.concat(...value);
          return [(0, import_utils.isArray)(value) ? (0, import_utils.last)(value) : value].filter(Boolean);
        });
      },
      deselectNode({ context, event }) {
        const value = (0, import_utils.toArray)(event.id || event.value);
        context.set("selectedValue", (prev) => (0, import_utils.remove)(prev, ...value));
      },
      ...
      expandBranch(params) {
        const { event } = params;
        (0, import_expand_branch.expandBranches)(params, [event.id]);
      },
      expandBranches(params) {
        const { context, event } = params;
        const valuesToExpand = (0, import_utils.toArray)(event.value);
        (0, import_expand_branch.expandBranches)(params, (0, import_utils.diff)(valuesToExpand, context.get("expandedValue")));
      },
      collapseBranch({ context, event }) {
        context.set("expandedValue", (prev) => (0, import_utils.remove)(prev, event.id));
      },
      collapseBranches(params) {
        const { context, event } = params;
        const value = (0, import_utils.toArray)(event.value);
        context.set("expandedValue", (prev) => (0, import_utils.remove)(prev, ...value));
      },
      setExpanded({ context, event }) {
        if (!(0, import_utils.isArray)(event.value)) return;
      ...
      focusTreeFirstNode(params) {
        ...
        const firstNode = collection2.getFirstNode(void 0, { skip: (0, import_visit_skip.skipFn)(params) });
        if (!firstNode) return;
        const firstValue = collection2.getNodeValue(firstNode);
        const scrolled = scrollToNode(params, firstValue);
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, firstValue));
        else dom.focusNode(scope, firstValue);
      },
      focusTreeLastNode(params) {
        const { prop, scope } = params;
        const collection2 = prop("collection");
        const lastNode = collection2.getLastNode(void 0, { skip: (0, import_visit_skip.skipFn)(params) });
        const lastValue = collection2.getNodeValue(lastNode);
        const scrolled = scrollToNode(params, lastValue);
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, lastValue));
      ...
      focusBranchFirstNode(params) {
        ...
        const firstNode = collection2.getFirstNode(branchNode, { skip: (0, import_visit_skip.skipFn)(params) });
        if (!firstNode) return;
        const firstValue = collection2.getNodeValue(firstNode);
        const scrolled = scrollToNode(params, firstValue);
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, firstValue));
      ...
      focusTreeNextNode(params) {
        const { event, prop, scope } = params;
        const collection2 = prop("collection");
        const nextNode = collection2.getNextNode(event.id, { skip: (0, import_visit_skip.skipFn)(params) });
        if (!nextNode) return;
        const nextValue = collection2.getNodeValue(nextNode);
        const scrolled = scrollToNode(params, nextValue);
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, nextValue));
        else dom.focusNode(scope, nextValue);
      },
      focusTreePrevNode(params) {
        const { event, prop, scope } = params;
        const collection2 = prop("collection");
        const prevNode = collection2.getPreviousNode(event.id, { skip: (0, import_visit_skip.skipFn)(params) });
        if (!prevNode) return;
        const prevValue = collection2.getNodeValue(prevNode);
        const scrolled = scrollToNode(params, prevValue);
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, prevValue));
      ...
      focusBranchNode(params) {
        ...
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, parentValue));
      ...
      focusMatchedNode(params) {
        ...
        const node = (0, import_dom_query.getByTypeahead)(elements, {
        ...
        if (scrolled) (0, import_dom_query.raf)(() => dom.focusNode(scope, node.id));
        else dom.focusNode(scope, node.id);
      },
      toggleNodeSelection({ context, event }) {
        const selectedValue = (0, import_utils.addOrRemove)(context.get("selectedValue"), event.id);
        context.set("selectedValue", selectedValue);
      },
      expandAllBranches(params) {
        const { context, prop } = params;
        const branchValues = prop("collection").getBranchValues();
        const valuesToExpand = (0, import_utils.diff)(branchValues, context.get("expandedValue"));
        (0, import_expand_branch.expandBranches)(params, valuesToExpand);
      },
      expandSiblingBranches(params) {
        ...
        const valuesToExpand = (0, import_utils.diff)(values, context.get("expandedValue"));
        (0, import_expand_branch.expandBranches)(params, valuesToExpand);
      },
      extendSelectionToNode(params) {
        const { context, event, prop, computed } = params;
        const collection2 = prop("collection");
        const anchorValue = (0, import_utils.first)(context.get("selectedValue")) || collection2.getNodeValue(collection2.getFirstNode());
        ...
        context.set("selectedValue", (0, import_utils.uniq)(values));
      },
      ...
      extendSelectionToFirstNode(params) {
        const { context, prop } = params;
        const collection2 = prop("collection");
        const currentSelection = (0, import_utils.first)(context.get("selectedValue"));
        const values = [];
        collection2.visit({
          skip: (0, import_visit_skip.skipFn)(params),
          onEnter: (node) => {
            const nodeValue = collection2.getNodeValue(node);
      ...
      toggleChecked({ context, event, prop }) {
        ...
        context.set(
          "checkedValue",
          (prev) => event.isBranch ? (0, import_checked_state.toggleBranchChecked)(collection2, event.value, prev) : (0, import_utils.addOrRemove)(prev, event.value)
        );
      },
      ...
      syncRenameInput({ context, scope, prop }) {
        ...
        (0, import_dom_query.setElementValue)(inputEl, label);
      },
