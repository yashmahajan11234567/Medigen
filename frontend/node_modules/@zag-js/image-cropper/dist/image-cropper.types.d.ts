...
import { RequiredBy, DirectionProperty, CommonProperties, Rect, Size, Point, PropTypes } from '@zag-js/types';
...
interface ZoomChangeDetails {
    zoom: number;
}
interface RotationChangeDetails {
    rotation: number;
}
interface FlipState {
    horizontal: boolean;
...
interface FlipChangeDetails {
    flip: FlipState;
}
interface CropChangeDetails {
    crop: Rect;
}
interface PreviewDescriptionDetails {
    zoom: number | null;
    rotation: number | null;
    crop: Rect;
}
interface SelectionLabelDetails {
    shape: "rectangle" | "circle";
}
interface SelectionValueTextDetails extends Rect {
    shape: "rectangle" | "circle";
}
...
interface ImageCropperProps extends DirectionProperty, CommonProperties {
    /**
...
type PropsWithDefault = "minWidth" | "minHeight" | "defaultZoom" | "defaultRotation" | "defaultFlip" | "zoomStep" | "zoomSensitivity" | "minZoom" | "maxZoom" | "maxWidth" | "maxHeight" | "fixedCropArea" | "cropShape" | "nudgeStep" | "nudgeStepShift" | "nudgeStepCtrl" | "translations";
interface ImageCropperSchema {
...
type ImageCropperService = Service<ImageCropperSchema>;
type ImageCropperMachine = Machine<ImageCropperSchema>;
...
interface ImageCropperApi<T extends PropTypes = PropTypes> {
    /**
