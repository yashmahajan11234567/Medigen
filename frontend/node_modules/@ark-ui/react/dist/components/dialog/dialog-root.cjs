"use client";
const require_render_strategy = require("../../utils/render-strategy.cjs");
const require_split_presence_props = require("../presence/split-presence-props.cjs");
const require_use_presence = require("../presence/use-presence.cjs");
const require_use_presence_context = require("../presence/use-presence-context.cjs");
const require_use_dialog_context = require("./use-dialog-context.cjs");
const require_use_dialog = require("./use-dialog.cjs");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/dialog/dialog-root.tsx
var DialogRoot = (props) => {
	const [presenceProps, { children, ...localProps }] = require_split_presence_props.splitPresenceProps(props);
	const [renderStrategyProps] = require_render_strategy.splitRenderStrategyProps(presenceProps);
	const dialog = require_use_dialog.useDialog(localProps);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_use_dialog_context.DialogProvider, {
		value: dialog,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_render_strategy.RenderStrategyPropsProvider, {
			value: renderStrategyProps,
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_use_presence_context.PresenceProvider, {
				value: require_use_presence.usePresence((0, _zag_js_react.mergeProps)({ present: dialog.open }, presenceProps)),
				children
			})
		})
	});
};
//#endregion
exports.DialogRoot = DialogRoot;
