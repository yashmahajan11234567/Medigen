"use client";
const require_factory = require("../factory.cjs");
const require_use_presence_context = require("../presence/use-presence-context.cjs");
const require_use_popover_context = require("./use-popover-context.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/popover/popover-positioner.tsx
var PopoverPositioner = (0, react.forwardRef)((props, ref) => {
	const popover = require_use_popover_context.usePopoverContext();
	const presence = require_use_presence_context.usePresenceContext();
	const mergedProps = (0, _zag_js_react.mergeProps)(popover.getPositionerProps(), props);
	if (presence.unmounted) return null;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
		...mergedProps,
		ref
	});
});
PopoverPositioner.displayName = "PopoverPositioner";
//#endregion
exports.PopoverPositioner = PopoverPositioner;
