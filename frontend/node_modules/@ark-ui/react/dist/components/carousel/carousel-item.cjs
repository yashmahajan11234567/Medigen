"use client";
const require_create_split_props = require("../../utils/create-split-props.cjs");
const require_factory = require("../factory.cjs");
const require_use_carousel_context = require("./use-carousel-context.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/carousel/carousel-item.tsx
var splitItemProps = require_create_split_props.createSplitProps();
var CarouselItem = (0, react.forwardRef)((props, ref) => {
	const [itemProps, localProps] = splitItemProps(props, ["index", "snapAlign"]);
	const mergedProps = (0, _zag_js_react.mergeProps)(require_use_carousel_context.useCarouselContext().getItemProps(itemProps), localProps);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
		...mergedProps,
		ref
	});
});
CarouselItem.displayName = "CarouselItem";
//#endregion
exports.CarouselItem = CarouselItem;
