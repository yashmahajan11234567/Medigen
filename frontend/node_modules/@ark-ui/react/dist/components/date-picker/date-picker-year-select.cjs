"use client";
const require_factory = require("../factory.cjs");
const require_use_date_picker_context = require("./use-date-picker-context.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/date-picker/date-picker-year-select.tsx
var DatePickerYearSelect = (0, react.forwardRef)((props, ref) => {
	const datePicker = require_use_date_picker_context.useDatePickerContext();
	const mergedProps = (0, _zag_js_react.mergeProps)(datePicker.getYearSelectProps(), props);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.select, {
		...mergedProps,
		ref,
		children: datePicker.getYears().map((year, i) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
			value: year.value,
			children: year.label
		}, i))
	});
});
DatePickerYearSelect.displayName = "DatePickerYearSelect";
//#endregion
exports.DatePickerYearSelect = DatePickerYearSelect;
