"use client";
const require_factory = require("../factory.cjs");
const require_use_drawer_context = require("./use-drawer-context.cjs");
let react = require("react");
let _zag_js_react = require("@zag-js/react");
let react_jsx_runtime = require("react/jsx-runtime");
//#region src/components/drawer/drawer-swipe-area.tsx
var DrawerSwipeArea = (0, react.forwardRef)((props, ref) => {
	const mergedProps = (0, _zag_js_react.mergeProps)(require_use_drawer_context.useDrawerContext().getSwipeAreaProps(), props);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_factory.ark.div, {
		...mergedProps,
		ref
	});
});
DrawerSwipeArea.displayName = "DrawerSwipeArea";
//#endregion
exports.DrawerSwipeArea = DrawerSwipeArea;
